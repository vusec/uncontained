...
/* list_entry is using a different struct type than preceeding list_add */
...
{
    struct test1 *test1 = list_entry(head, struct test1, list);
    printf("test: %p\n", test1);
}
...
struct test1 test1_decoy_storage;

int main(int argc, char*argv[]) {
    func1();

    // break optimizations
    list_add(&test1_decoy_storage.list, &test_list_decoy);
