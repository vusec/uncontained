...
/* the container_of is incorrect since kobj is actuallly within outer_container */

static ssize_t show(struct kobject *kobj, struct attribute *attr, char *buf)
{
    struct outer_container2 *other = container_of(kobj, struct outer_container2, container.kobj);
    printf("other: %p\n", other);
    return 0;
}
